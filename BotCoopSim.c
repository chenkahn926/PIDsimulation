...
for (k=0; k<attacker_num ; k++)
{
	...
	*/
	...
	for (i=0; i<NodeIndex; i++){
		if (nodes[i].attacker == 1 && i != attackerIndex){
			pointer = i;
...
/********************************   Attacking victim, case 1.   *************************************/
	...
	while (1){
		...
		if (nodes[victimIndex].attacker == 1)
			break;
	}
	...
	for (i=0; i<NodeIndex; i++){
		...
		if (nodes[i].attacker == 1  && i != victimIndex){
...
/********************************   Attacking victim, case 2.   *************************************/
	...
	for (i=0; i<NodeIndex; i++){
		...
		if (nodes[i].Dist == 1000 || nodes[i].Dist == 1000000)
			continue;
